...
<div class="container">
    ...
    function resolve_dns($host, $servers, $query_type) {
        ...
        foreach ($servers as $server) {
            ...
            $records = [];
            if ($query_type === 'PTR') {
                // Pour les requêtes PTR (reverse DNS)
                preg_match_all('/\sPTR\s+(.+)\.?$/', $output, $matches);
                $records = array_map('rtrim', $matches[1] ?? [], array_fill(0, count($matches[1] ?? []), '.'));
    ...
    if (filter_var($host_raw, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
        $query_type = 'PTR';
        // Conversion de l'IPv4 en format inverse
        $parts = explode('.', $host_raw);
        $host_for_query = implode('.', array_reverse($parts)) . '.in-addr.arpa';
        $host = escapeshellarg($host_for_query);
    } else if (filter_var($host_raw, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
        $query_type = 'PTR';
        // Conversion de l'IPv6 en format inverse
        $expanded = inet_pton($host_raw);
        $hex = unpack('H*hex', $expanded)['hex'];
        $reversed = implode('.', array_reverse(str_split($hex))) . '.ip6.arpa';
        $host_for_query = $reversed;
        $host = escapeshellarg($host_for_query);
    } else {
    ...
    <div class="section">
        <h4>DNS Publics</h4>
        <?php foreach ($dns_publics as $server => $data): ?>
            <div class="result">
                <strong>Serveur <?= $server ?> (<?= $data['time'] ?> ms)</strong>
                <?php if ($data['status'] === 'NOERROR' && !empty($data['records'])): ?>
                    <div class="success-result">✅ <?= implode(', ', $data['records']) ?></div>
                <?php else: ?>
                    <div class="error-result">❌ <?= $data['status'] ?></div>
                <?php endif; ?>
            </div>
        <?php endforeach; ?>
    </div>
    ...
    <div class="section">
        <h4>DNS Privés</h4>
        <?php foreach ($dns_prives as $server => $data): ?>
            <div class="result">
                <strong>Serveur <?= $server ?> (<?= $data['time'] ?> ms)</strong>
                <?php if ($data['status'] === 'NOERROR' && !empty($data['records'])): ?>
                    <div class="success-result">✅ <?= implode(', ', $data['records']) ?></div>
